...
exports.handler = function (event, context, callback) {
    ...
    alexa.APP_ID = APP_ID;
   alexa.dynamoDBTableName='nameorigin';
...
const WELCOME_MESSAGE = 'Welcome to ' + SKILL_NAME + '. You can ask a question like, give me a random eco tip?... Is a book recycable? or choose a challenge...Now, what can I help you with?';
...
const challenge_prompt='choose a challenge';
/*
...
const handlers = {
    ...
    */
    ...
    {   
        ...
        if (!filledSlots) {
            ...
            if (slotValues.location.value=="home") {
                const factArr = HOME_TIPS;
                const factIndex = Math.floor(Math.random() * factArr.length);
                const randomFact = factArr[factIndex];
                const speechOutput = GET_LOCATION_FACT_MESSAGE + slotValues.location.value  + " related tip ....." + randomFact;
                /*
                this.response.cardRenderer(SKILL_NAME, randomFact);
                this.response.speak(speechOutput);
                this.emit(':responseReady');
            ...
            else if(slotValues.location.value=="school")
            {
            ...
            else if(slotValues.location.value==="work")
            {
    ...
    */
    ...
    'challengesIntent': function(){ 
       
        const factArr = challenges;
        //Gets a random index to use on the data set of random facts
        const factIndex = Math.floor(Math.random() * factArr.length);
        //Gets a random fact
        const randomFact = factArr[factIndex];
        //Sets the speech output to have the 
        //const shouldEndSession = false;
        var speechOutput='your challenge for this week is '+randomFact;
        ...
        ddb.batchWriteItem(params, function(err, data) {
          if (err) {
            ...
            this.emit(':ask', "wrote it to the db").listen(); 
          }
        });
        
        this.emit(':ask', speechOutput, HELP_REPROMPT).listen(); 
